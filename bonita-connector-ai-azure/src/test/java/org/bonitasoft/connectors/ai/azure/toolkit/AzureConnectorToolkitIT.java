...
import org.bonitasoft.connectors.ai.azure.toolkit.AiConnectorTestToolkit.Output;
...
/**
 * Integration tests that deploy Azure OpenAI connectors into a real Bonita runtime
 ...
 * Requires:
 * - Docker running
 * - AZURE_OPENAI_API_KEY and AZURE_OPENAI_ENDPOINT environment variables set
...
class AzureConnectorToolkitIT {

    private static final String AZURE_ASK_DEF_ID = "azure-ask";
    private static final String AZURE_ASK_DEF_VERSION = "1.0.0";

    private static final String ARTIFACT_ID = "bonita-connector-ai-azure";

    private static final Logger LOGGER = LoggerFactory.getLogger(AzureConnectorToolkitIT.class);
    ...
    static void installOrganization() {
        ...
        var orgFile = AzureConnectorToolkitIT.class.getResource("/ACME.xml");
        if (orgFile != null) {
    ...
    void should_execute_ask_connector_in_bonita_process() throws Exception {
        ...
        var barFile = AiConnectorTestToolkit.buildConnectorToTest(
                AZURE_ASK_DEF_ID, AZURE_ASK_DEF_VERSION, inputs, outputs, ARTIFACT_ID);
