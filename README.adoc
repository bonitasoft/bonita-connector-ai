...
- *chatModelName*: The chat model to use (ex gpt-4o, gpt-4o-mini, llama3, ...)
...
- *sourceDocumentRef*: The reference to the process document to load and add to the user prompt. If not null, the connector will try to read the specified document and replace a placeholder named `{document}` in the user prompt with its content.

AI result will be placed as a map entry for the key named *output*.
