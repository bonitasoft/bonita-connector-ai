...
/**
 * Property-based tests for Anthropic connector using jqwik.
 * Tests configuration building and chat model creation with randomly generated data.
 */
class AnthropicConnectorPropertyTest {
    ...
    @Property(tries = 50)
    @Label("Valid base URL should always create a chat model")
    void should_create_model_when_base_url_valid(
            @ForAll @AlphaChars @StringLength(min = 1, max = 100) String urlSuffix) {
        String baseUrl = "https://" + urlSuffix + ".example.com";
        AiConfiguration config =
                AiConfiguration.builder().apiKey("test-key").baseUrl(baseUrl).build();
        AnthropicAskAiChat chat = new AnthropicAskAiChat(config);

        assertThat(chat.getChatModel()).isNotNull();
        assertThat(config.getBaseUrl()).hasValue(baseUrl);
    }
    ...
    void should_create_all_chat_types_with_same_config(
        ...
        assertThat(new AnthropicAskAiChat(config).getChatModel()).isNotNull();
        assertThat(new AnthropicClassifyChat(config).getChatModel()).isNotNull();
        assertThat(new AnthropicExtractChat(config).getChatModel()).isNotNull();
    }
