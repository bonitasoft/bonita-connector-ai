...
/**
 * Property-based tests for Gemini connector using jqwik.
 * Tests configuration building and chat model creation with randomly generated data.
 */
class GeminiConnectorPropertyTest {
    ...
    void should_create_all_chat_types_with_same_config(
        ...
        assertThat(new GeminiAskAiChat(config).getChatModel()).isNotNull();
        assertThat(new GeminiClassifyChat(config).getChatModel()).isNotNull();
        assertThat(new GeminiExtractChat(config).getChatModel()).isNotNull();
    }
