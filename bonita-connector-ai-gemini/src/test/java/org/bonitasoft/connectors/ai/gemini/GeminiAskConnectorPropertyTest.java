...
/**
 * Property-based tests specific to the Ask operation of the Gemini connector.
 * Validates that GeminiAskAiChat handles random valid inputs correctly.
 */
class GeminiAskConnectorPropertyTest {
    ...
    void systemPromptOptional(@ForAll @AlphaChars @StringLength(min = 1, max = 50) String apiKey) {
        // systemPrompt is not part of AiConfiguration (it is an input param), but config builds fine without it
        AiConfiguration config = AiConfiguration.builder().apiKey(apiKey).build();
        GeminiAskAiChat chat = new GeminiAskAiChat(config);
    ...
    void userPromptMandatory(@ForAll @AlphaChars @StringLength(min = 1, max = 50) String apiKey) {
        // userPrompt is validated at execution time, not at config build time
        AiConfiguration config = AiConfiguration.builder().apiKey(apiKey).build();
        GeminiAskAiChat chat = new GeminiAskAiChat(config);
    ...
    void jsonSchemaOptional(@ForAll @AlphaChars @StringLength(min = 1, max = 50) String apiKey) {
        // outputJsonSchema is an input param, not part of AiConfiguration
        AiConfiguration config = AiConfiguration.builder().apiKey(apiKey).build();
        GeminiAskAiChat chat = new GeminiAskAiChat(config);
    ...
    @Property(tries = 50)
    @Label("Debug logging both true and false should be accepted for Ask chat")
    ...
    void baseUrlOptional(@ForAll @AlphaChars @StringLength(min = 1, max = 50) String apiKey) {
        ...
        assertThat(new GeminiAskAiChat(configWithout).getChatModel()).isNotNull();
        assertThat(new GeminiAskAiChat(configWith).getChatModel()).isNotNull();
