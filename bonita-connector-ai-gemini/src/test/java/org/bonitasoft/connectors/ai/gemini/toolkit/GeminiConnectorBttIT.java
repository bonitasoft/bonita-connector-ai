 ...
 */
package org.bonitasoft.connectors.ai.gemini.toolkit;
 ...
 * </ul>
 *
 * <p>Requires: Docker running, GEMINI_API_KEY env var set
 *
 * @see GeminiConnectorStudioIT for running against Bonita Studio
 */
@Slf4j
@Tag("btt")
@Testcontainers
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
class GeminiConnectorBttIT {

    private static final String PROCESS_NAME = "AI_CONNECTOR_TEST";
    private static final String CONNECTOR_DEF_ID = "gemini-ask";
    private static final String CONNECTOR_DEF_VERSION = "1.0.0";
    private static final String ARTIFACT_ID = "bonita-connector-ai-gemini";

    private static final Logger LOGGER = LoggerFactory.getLogger(GeminiConnectorBttIT.class);
    ...
    static void setUp() throws Exception {
        ...
        var orgFile = GeminiConnectorBttIT.class.getResource("/ACME.xml");
        if (orgFile != null) {
            bonitaClient
                    .users()
        ...
        toolkit = BonitaTestToolkitFactory.INSTANCE.get(GeminiConnectorBttIT.class);
    }
    ...
    /**
     * Builds a BAR with the Gemini Ask connector and deploys it to the target Bonita runtime.
     */
    static void deployConnectorProcess(BonitaClient client) throws Exception {
        ...
        inputs.put("apiKey", System.getenv("GEMINI_API_KEY"));
        ...
        File barFile = null;
        try {
            barFile = Files.createTempFile("gemini-btt-test", ".bar").toFile();
            barFile.delete();
