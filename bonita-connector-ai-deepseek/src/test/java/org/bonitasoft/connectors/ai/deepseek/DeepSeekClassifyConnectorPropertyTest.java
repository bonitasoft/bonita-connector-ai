...
/**
 * Property-based tests specific to the Classify operation of the DeepSeek connector.
 * Validates that DeepSeekClassifyChat handles random valid inputs correctly.
 */
class DeepSeekClassifyConnectorPropertyTest {
    ...
    void modelNameOptional(@ForAll @AlphaChars @StringLength(min = 1, max = 100) String modelName) {
        ...
        assertThat(new DeepSeekClassifyChat(configWith).getChatModel()).isNotNull();
        assertThat(new DeepSeekClassifyChat(configWithout).getChatModel()).isNotNull();
