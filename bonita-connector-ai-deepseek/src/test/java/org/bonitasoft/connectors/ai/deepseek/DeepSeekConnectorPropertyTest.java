...
/**
 * Property-based tests for DeepSeek connector using jqwik.
 * Tests configuration building and chat model creation with randomly generated data.
 */
class DeepSeekConnectorPropertyTest {
    ...
    void should_create_all_chat_types_with_same_config(
        ...
        assertThat(new DeepSeekAskAiChat(config).getChatModel()).isNotNull();
        assertThat(new DeepSeekClassifyChat(config).getChatModel()).isNotNull();
        assertThat(new DeepSeekExtractChat(config).getChatModel()).isNotNull();
    }
