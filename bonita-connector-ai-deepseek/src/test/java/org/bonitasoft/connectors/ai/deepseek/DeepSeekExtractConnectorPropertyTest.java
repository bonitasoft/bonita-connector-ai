...
/**
 * Property-based tests specific to the Extract operation of the DeepSeek connector.
 * Validates that DeepSeekExtractChat handles random valid inputs correctly.
 */
class DeepSeekExtractConnectorPropertyTest {
    ...
    void modelNameOptional(@ForAll @AlphaChars @StringLength(min = 1, max = 100) String modelName) {
        ...
        assertThat(new DeepSeekExtractChat(configWith).getChatModel()).isNotNull();
        assertThat(new DeepSeekExtractChat(configWithout).getChatModel()).isNotNull();
