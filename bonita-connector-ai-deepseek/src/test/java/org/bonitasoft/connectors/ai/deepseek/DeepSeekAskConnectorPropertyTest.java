...
/**
 * Property-based tests specific to the Ask operation of the DeepSeek connector.
 * Validates that DeepSeekAskAiChat handles random valid inputs correctly.
 */
class DeepSeekAskConnectorPropertyTest {
    ...
    void baseUrlOptional(@ForAll @AlphaChars @StringLength(min = 1, max = 50) String apiKey) {
        ...
        assertThat(new DeepSeekAskAiChat(configWithout).getChatModel()).isNotNull();
        assertThat(new DeepSeekAskAiChat(configWith).getChatModel()).isNotNull();
