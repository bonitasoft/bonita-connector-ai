...
/**
 * Property-based tests for Groq connector using jqwik.
 * Tests configuration building and chat model creation with randomly generated data.
 */
class GroqConnectorPropertyTest {
    ...
    void should_create_all_chat_types_with_same_config(
        ...
        assertThat(new GroqAskAiChat(config).getChatModel()).isNotNull();
        assertThat(new GroqClassifyChat(config).getChatModel()).isNotNull();
        assertThat(new GroqExtractChat(config).getChatModel()).isNotNull();
    }
