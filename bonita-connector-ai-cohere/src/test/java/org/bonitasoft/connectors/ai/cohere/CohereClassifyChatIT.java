...
class CohereClassifyChatIT extends ClassifyChatIT {

    @Override
    protected void customize(AiConfiguration.AiConfigurationBuilder builder) {
        builder.apiKey(System.getenv("COHERE_API_KEY"));
        builder.baseUrl(CohereChat.DEFAULT_BASE_URL);
        builder.chatModelName(CohereChat.DEFAULT_MODEL);
    }

    @Override
    protected ClassifyChat getChat(AiConfiguration configuration) {
        return new CohereClassifyChat(configuration);
    }
}
